...
export function ProductGroupsPage() {
  ...
  function startEdit(group: Tables<"product_groups">) {
    setForm({
      id: group.id,
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const { error: saveError } = form.id
      ? await supabase.from("product_groups").update(payload).eq("id", form.id)
      : await supabase.from("product_groups").insert(payload);
    setSaving(false);

    if (saveError) {
      console.error("Save product group failed:", saveError);
    ...
    <div className="flex flex-1 flex-col gap-6 p-6">
        ...
        <table className="w-full min-w-max text-sm">
          ...
          <tbody>
            ...
            {groups.map((group) => (
              ...
              >
                <td className="p-2">{group.name_en}</td>
                <td className="p-2" dir="auto">
                  {group.name_ar}
                </td>
                <td className="p-2 text-end">{group.sort_order}</td>
                <td className="p-2 text-center">{group.active ? "✓" : ""}</td>
      ...
      <form onSubmit={handleSubmit} className="flex max-w-md flex-col gap-4">
        <h2 className="text-base font-semibold">
          {form.id ? t.editValue : t.newProductGroupTitle}
        </h2>
